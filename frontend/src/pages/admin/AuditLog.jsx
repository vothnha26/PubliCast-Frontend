    ...
    <div className="flex-1 overflow-y-auto bg-background" style={{ padding: "40px 60px" }}>
      ...
      <div className="flex items-start justify-between mb-10">
        <div className="flex gap-5">
           ...
           <div>
              ...
              <p className="text-muted-foreground mt-1">Immutable record of all administrative and user actions across the platform.</p>
           </div>
        </div>
        <div className="flex gap-3">
           <button className="flex items-center gap-2 px-5 py-2.5 bg-card border border-border rounded-xl text-xs font-bold text-muted-foreground hover:bg-muted transition-all">
      ...
      <div className="mb-10 p-6 rounded-[32px] bg-red-50 border border-red-100 flex items-center justify-between animate-pulse">
         ...
         <div className="flex gap-3">
            ...
            <button className="px-5 py-2 bg-card text-red-600 border border-red-200 rounded-xl text-xs font-bold">Investigate</button>
         </div>
      </div>
      ...
      <div className="bg-card rounded-[40px] border border-border shadow-sm overflow-hidden">
        <div className="px-8 py-6 border-b border-gray-50 flex items-center justify-between bg-muted/30 flex-wrap gap-4">
           <div className="flex items-center gap-6">
              {["All", "Security", "Billing", "Team", "Content"].map(f => (
                <button 
                  key={f} 
                  onClick={() => updateFilters({ category: f })}
                  className={`text-[10px] font-black uppercase tracking-widest transition-all ${activeFilter === f ? "text-black border-b-2 border-black pb-1" : "text-muted-foreground hover:text-muted-foreground"}`}
                >
                  {f}
                </button>
              ))}
           ...
           <div className="flex items-center gap-3">
              ...
              <select
                ...
                className="text-xs bg-card border border-border rounded-xl outline-none focus:border-black cursor-pointer transition-all"
              ...
              <div className="relative">
                 ...
                 <input 
                   ...
                   className="pl-9 pr-4 py-2 bg-card border border-border rounded-xl text-xs outline-none focus:border-black w-64 transition-all" 
                 />
              </div>
           </div>
        </div>
                ...
                <tr>
                  <td colSpan={6} className="px-8 py-20 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <span className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Loading logs...</span>
                    </div>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={6} className="px-8 py-10 text-center">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <span className="text-xs text-muted-foreground font-bold uppercase tracking-widest">No logs match criteria</span>
                    </div>
                  </td>
                </tr>
                ...
                return (
                  <React.Fragment key={entry.id}>
                    <tr 
                      ...
                      className={`hover:bg-muted/50 transition-colors cursor-pointer group ${entry.status === 'failed' ? 'bg-red-50/30' : ''}`}
                    >
                       <td className="px-8 py-5">
                          <div className="text-xs font-bold text-foreground">
                            {new Date(entry.createdAt).toLocaleDateString('vi-VN', { year: 'numeric', month: 'short', day: 'numeric' })}
                          </div>
                          <div className="text-[10px] text-muted-foreground mt-0.5 font-mono">{entry.time}</div>
                       </td>
                       <td className="px-8 py-5">
                          <div className="flex items-center gap-3">
                             <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center font-bold text-[10px] text-muted-foreground">{entry.actor[0]}</div>
                             <div>
                                <div className="text-xs font-bold text-foreground">{entry.actor}</div>
                                <div className="text-[9px] text-muted-foreground font-bold uppercase tracking-tighter">{entry.role} · {entry.ip}</div>
                             </div>
                          </div>
                       </td>
                       <td className="px-8 py-5">
                          <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-tighter ${categoryColors[entry.category] || "bg-muted text-foreground"}`}>
                             {entry.category}
                          </span>
                       </td>
                       <td className="px-8 py-5">
                          <div className="text-xs font-bold text-foreground">{entry.action}</div>
                          <div className="text-[10px] text-muted-foreground truncate max-w-[150px]">{entry.target}</div>
                       </td>
                       <td className="px-8 py-5">
                    ...
                    {isExpanded && (
                      <tr>
                         <td colSpan={6} className="bg-muted/80 px-8 py-8 animate-in slide-in-from-top-2 duration-300">
                            <div className="grid grid-cols-3 gap-8">
                               <div className="space-y-4">
                                  ...
                                  <div className="space-y-2">
                                     <div className="flex justify-between text-[11px]"><span className="text-muted-foreground">Request ID</span><span className="font-mono font-bold text-foreground">REQ-9428-AB-{entry.id.substring(0, 8)}</span></div>
                                     <div className="flex justify-between text-[11px]"><span className="text-muted-foreground">Trace Mode</span><span className="font-bold text-blue-600">STRICT-COMPLIANCE</span></div>
                                     <div className="flex justify-between text-[11px]"><span className="text-muted-foreground">Latency</span><span className="font-bold text-foreground">42ms</span></div>
                                  </div>
                               </div>
                               <div className="space-y-4">
                                  <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-2">
                                  ...
                                  <div className="space-y-2">
                                     <div className="flex justify-between text-[11px]"><span className="text-muted-foreground">Node</span><span className="font-bold text-foreground">AWS-SEA-042</span></div>
                                     <div className="flex justify-between text-[11px]"><span className="text-muted-foreground">Env</span><span className="font-bold text-green-600 uppercase">Production</span></div>
                                  </div>
                               </div>
                               <div className="space-y-4">
                                  <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest flex items-center gap-2">
        ...
        {/* Footer / Pagination */}
        <div className="px-8 py-5 border-t border-gray-50 flex items-center justify-between bg-muted/20">
           ...
           <div className="flex items-center gap-2">
              <button 
                ...
                className="px-4 py-2 bg-card border border-border rounded-xl text-[10px] font-bold text-muted-foreground hover:bg-muted transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Previous
              </button>
              <button 
