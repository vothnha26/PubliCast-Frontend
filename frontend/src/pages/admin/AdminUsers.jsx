...
export function AdminUsers() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-background" style={{ padding: "40px 60px", fontFamily: "'DM Sans', sans-serif" }}>
      {/* Header */}
      <div className="flex items-start justify-between mb-10">
        <div className="flex gap-5">
          ...
          <div>
            ...
            <p className="text-muted-foreground mt-1">Quản lý tài khoản người dùng hệ thống, phân quyền và kiểm soát quyền truy cập.</p>
          </div>
        </div>
      </div>
        ...
        <div className="px-8 py-6 border-b border-gray-50 flex items-center justify-between bg-muted/30 flex-wrap gap-4">
          <div className="flex items-center gap-6">
            ...
            <select
              ...
              className="text-xs bg-card border border-border rounded-xl outline-none focus:border-black cursor-pointer transition-all px-3 py-1.5"
            >
            ...
            {loading ? (
              <tr>
                <td colSpan={6} className="px-8 py-20 text-center">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <span className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Đang tải người dùng...</span>
                  </div>
                </td>
              </tr>
            ) : usersList.length === 0 ? (
              <tr>
                <td colSpan={6} className="px-8 py-10 text-center">
                  <div className="flex flex-col items-center justify-center">
                    ...
                    <span className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Không có người dùng nào khớp điều kiện</span>
                  </div>
                </td>
              </tr>
            ) : usersList.map((user) => (
              <tr key={user.id} className="hover:bg-muted/50 transition-colors">
                ...
                <td className="px-8 py-5">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center font-bold text-[10px] text-muted-foreground overflow-hidden">
                      {user.avatarUrl ? (
                ...
                <td className="px-8 py-5">
                  <select
                    ...
                    className="text-xs bg-transparent border border-border rounded-lg outline-none focus:border-black cursor-pointer font-bold px-2 py-1"
                  >
                    {SYSTEM_ROLES.map(r => (
                      <option key={r} value={r}>{r}</option>
                    ))}
