...
export function PerformanceOverviewWidget({
  ...
  if (isLoading) {
    return (
      <div className="bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-800 rounded-3xl p-6 space-y-6 shadow-sm">
        {/* Header Skeleton */}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-2xl p-4 space-y-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          <div className="lg:col-span-2 bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-2xl p-4 space-y-3">
          ...
          <div className="lg:col-span-3 bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-800 rounded-2xl p-4 space-y-2">
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-gray-150 dark:border-slate-800 rounded-3xl p-6 space-y-6 shadow-sm hover:shadow-md transition-shadow">
      {/* Widget Header */}
      <div className="flex justify-between items-center pb-4 border-b border-gray-100 dark:border-slate-800">
        <h3 className="text-base font-extrabold text-gray-800 dark:text-white uppercase tracking-tight flex items-center gap-2">
          <Layers size={18} className="text-[#3B82F6]" />
          Tổng Quan Đa Kênh (Performance Overview)
        </h3>
        <span className="text-[10px] font-mono text-gray-400 dark:text-gray-500 font-bold tracking-widest uppercase">
          {period}
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div
            key={i}
            className="bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-850 rounded-2xl p-4 transition-all duration-300 hover:shadow-sm"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-850 rounded-2xl p-4.5 flex flex-col justify-between">
          <div>
            <span className="text-[10px] font-extrabold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3 block">
              Kênh Hoạt Động
            </span>
                ...
                channels.map((ch, idx) => (
                  <div key={idx} className="flex justify-between items-center text-xs">
                    <div className="flex items-center gap-2 truncate">
                      ...
                      <span className="font-bold text-gray-700 dark:text-gray-300 truncate w-32">
                        {ch.displayName}
                      </span>
                    </div>
                    <span className="font-mono font-extrabold text-gray-500 dark:text-gray-400">
                      {formatNumber(ch.followers)}
                    </span>
                  </div>
                ))
              ) : (
                <div className="text-xs text-gray-400 text-center py-4">Chưa liên kết kênh nào</div>
              )}
            </div>
          </div>
        </div>
        ...
        {/* Chart Trend Area */}
        <div className="lg:col-span-3 bg-gray-50 dark:bg-slate-950 border border-gray-100 dark:border-slate-850 rounded-2xl p-4.5 flex flex-col justify-between">
          <span className="text-[10px] font-extrabold text-gray-400 dark:text-gray-500 uppercase tracking-wider block mb-2">
