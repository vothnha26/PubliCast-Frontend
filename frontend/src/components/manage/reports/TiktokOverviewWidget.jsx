    ...
    return (
      <div className="w-full h-full flex flex-col justify-between p-2">
        <div className="flex flex-col items-center justify-center gap-2 text-center py-4 bg-muted border border-dashed border-border rounded-xl">
          ...
          <span className="text-xs font-bold text-muted-foreground">TIKTOK PERFORMANCE — Đang tải...</span>
        </div>
  ...
  return (
    <div className="w-full h-full flex flex-col justify-between overflow-hidden">
      ...
      <div className="grid grid-cols-4 gap-3 mb-2.5">
        <div className="p-2 bg-gradient-to-br from-white to-gray-50/50 border border-border rounded-xl shadow-sm hover:shadow transition-all duration-300" style={{ borderTop: `2px solid #000000` }}>
          <span className="text-[6.5px] text-gray-450 font-bold uppercase tracking-wider block">Followers</span>
          <div className="text-xs font-black text-foreground font-mono mt-0.5">{formatNumber(channel.followers)}</div>
        </div>
        <div className="p-2 bg-gradient-to-br from-white to-gray-50/50 border border-border rounded-xl shadow-sm hover:shadow transition-all duration-300" style={{ borderTop: `2px solid #FE2C55` }}>
        ...
        <div className="p-2 bg-gradient-to-br from-white to-gray-50/50 border border-border rounded-xl shadow-sm hover:shadow transition-all duration-300" style={{ borderTop: `2px solid #25F4EE` }}>
        ...
        <div className="p-2 bg-gradient-to-br from-white to-gray-50/50 border border-border rounded-xl shadow-sm hover:shadow transition-all duration-300" style={{ borderTop: `2px solid ${color}` }}>
      ...
      <div className="grid grid-cols-5 gap-3 items-stretch flex-1 min-h-0">
        ...
        <div className="col-span-2 bg-gradient-to-br from-white to-gray-50/30 border border-border rounded-xl p-2.5 flex flex-col shadow-sm">
          <span className="text-[7px] font-extrabold text-muted-foreground uppercase tracking-wider mb-2 block">Lượt xem theo tuần</span>
        ...
        <div className="col-span-3 bg-gradient-to-br from-white to-gray-50/30 border border-border rounded-xl p-3 flex flex-col justify-between shadow-sm">
          ...
          <div className="space-y-1.5 overflow-y-auto max-h-[110px] pr-1">
            {topVideos.length > 0 ? topVideos.map((post, idx) => (
              <div 
                key={post.id || idx} 
                className="border-b border-border pb-1 last:border-b-0 last:pb-0 hover:bg-muted/50 p-1 rounded transition-all duration-200"
              >
