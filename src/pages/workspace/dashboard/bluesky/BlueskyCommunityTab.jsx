...
import { Users, UserCheck, MessageSquare, ShieldCheck, CheckCircle2, Globe, Sparkles } from "lucide-react";

export function BlueskyCommunityTab({
  metrics,
  realData,
  communityGrowthData = []
}) {
  ...
  const totalContentInPeriod = communityGrowthData?.reduce((acc, curr) => acc + (curr.totalContent || 0), 0) || postsCount;
  ...
  const followersPerPost = postsCount > 0 ? (followersCount / postsCount).toFixed(2) : "0";
  ...
  const summaryGrid = [
    ...
    { label: "Following", value: followsCount.toLocaleString() },
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden bg-gradient-to-r from-blue-600 via-sky-500 to-indigo-600 rounded-3xl p-6 text-white shadow-xl shadow-blue-500/10">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="relative">
              {avatarUrl ? (
                ...
                <div className="w-16 h-16 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center text-2xl font-black border-2 border-white/30">
                  {displayName.substring(0, 2).toUpperCase()}
                </div>
              )}
              <div className="absolute -bottom-1 -right-1 bg-white p-1 rounded-full text-[#0085FF] shadow-sm">
                <Globe size={14} />
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {isEmailConfirmed && (
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-white/20 backdrop-blur-md text-white border border-white/30">
          ...
          <div className="flex items-center gap-3 bg-white/10 backdrop-blur-md p-3 rounded-2xl border border-white/20 w-full md:w-auto justify-around">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-card p-5 rounded-2xl border border-border shadow-sm flex items-center justify-between">
          ...
          <div className="w-12 h-12 rounded-2xl bg-blue-50 dark:bg-blue-950/40 text-[#0085FF] flex items-center justify-center">
            <Users size={22} />
          </div>
        </div>
        ...
        <div className="bg-card p-5 rounded-2xl border border-border shadow-sm flex items-center justify-between">
          ...
          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isEmailConfirmed ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600' : 'bg-amber-50 dark:bg-amber-950/40 text-amber-600'}`}>
            <ShieldCheck size={22} />
          </div>
        </div>
      </div>
      ...
      {/* Main Growth Chart */}
      <GenericDashboardTab
        title="Bluesky Growth & Community Analytics"
        description="Chỉ số phân tích tăng trưởng Followers, Following và bài viết trên mạng xã hội phân tán Bluesky"
        data={communityGrowthData.length > 0 ? communityGrowthData : (realData?.growth || [])}
