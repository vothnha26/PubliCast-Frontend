...
/**
 * Pure presentational Threads preset form — no context, no hooks beyond
 * translation. Receives everything via the single `preset` prop so both the
 * Post Composer and AutoList can render the exact same fields, each backed
 * by its own adapter hook (useThreadsPresetComposer / useThreadsPresetAutoList).
 */
export function ThreadsPresetFields({ preset }) {
  const { t } = useTranslation(["planner"]);
  const { isOpen, onToggleOpen, whoCanReply } = preset;
  ...
  return (
    <div className="border border-border rounded-3xl overflow-hidden bg-card shadow-sm transition-all duration-300">
      ...
      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-[300px] border-t border-gray-50 p-6' : 'max-h-0'}`}>
        <div className="space-y-4 text-left">
          <div>
            ...
            <div className="relative">
              <select
                value={whoCanReply.value}
                onChange={(e) => whoCanReply.onChange(e.target.value)}
