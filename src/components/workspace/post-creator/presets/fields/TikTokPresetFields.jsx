...
/**
 * Pure presentational TikTok preset form — shared by Post Composer
 * (useTikTokPresetComposer) and AutoList (useTikTokPresetAutoList).
 */
export function TikTokPresetFields({ preset }) {
  const { t } = useTranslation(["planner"]);
  const {
    isOpen, onToggleOpen,
    privacy, allowComments, allowDuet, allowStitch, aiGenerated, commercialContent,
  } = preset;
        ...
        <div className="space-y-5 text-left">
          ...
          <div className="flex items-center justify-between gap-4 pt-2 border-t border-gray-50">
            ...
            <div className="flex flex-col items-start gap-1">
              ...
              <button
                type="button"
                onClick={() => allowDuet.onChange(!allowDuet.value)}
                className={`w-10 h-6 flex items-center rounded-full p-1 cursor-pointer transition-all duration-300 ${
                  allowDuet.value ? 'bg-black' : 'bg-gray-200'
                }`}
              >
                <div
                  className={`bg-card w-4 h-4 rounded-full shadow-sm transform transition-all duration-300 ${
                    allowDuet.value ? 'translate-x-4' : 'translate-x-0'
                  }`}
                />
              </button>
            </div>
            <div className="flex flex-col items-start gap-1">
              ...
              <button
                type="button"
                onClick={() => allowStitch.onChange(!allowStitch.value)}
                className={`w-10 h-6 flex items-center rounded-full p-1 cursor-pointer transition-all duration-300 ${
                  allowStitch.value ? 'bg-black' : 'bg-gray-200'
                }`}
              >
                <div
                  className={`bg-card w-4 h-4 rounded-full shadow-sm transform transition-all duration-300 ${
                    allowStitch.value ? 'translate-x-4' : 'translate-x-0'
                  }`}
                />
              </button>
            </div>
          ...
          <div className="border-t border-border pt-4 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => aiGenerated.onChange(!aiGenerated.value)}
                className={`w-10 h-6 flex items-center rounded-full p-1 cursor-pointer transition-all duration-300 shrink-0 ${
                  aiGenerated.value ? 'bg-black' : 'bg-gray-200'
                }`}
              >
                <div
                  className={`bg-card w-4 h-4 rounded-full shadow-sm transform transition-all duration-300 ${
                    aiGenerated.value ? 'translate-x-4' : 'translate-x-0'
                  }`}
                />
              </button>
            </div>
            ...
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                type="button"
                onClick={() => commercialContent.onChange(!commercialContent.value)}
                className={`w-10 h-6 flex items-center rounded-full p-1 cursor-pointer transition-all duration-300 shrink-0 mt-0.5 ${
                  commercialContent.value ? 'bg-black' : 'bg-gray-200'
                }`}
              >
                <div
                  className={`bg-card w-4 h-4 rounded-full shadow-sm transform transition-all duration-300 ${
                    commercialContent.value ? 'translate-x-4' : 'translate-x-0'
                  }`}
                />
              </button>
            </div>
